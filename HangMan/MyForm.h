...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	// 1. Jeœli pole tekstowe jest puste, nic nie rób
	if (textBox1->Text->Length == 0) return;

	// Pobieramy literê i zamieniamy od razu na wielk¹
	...
	// 2. Sprawdzamy ka¿d¹ literê z wylosowanego s³owa
	for (int i = 0; i < wylosowaneSlo->Length; i++) {
		// Jeœli litera pasuje, odkrywamy j¹
		if (wylosowaneSlo[i] == litera) {
			noweWykryte += litera;
			trafiony = true;
		}
		else {
			// Jeœli nie, zostawiamy to co by³o (czyli literê lub podkreœlnik)
			noweWykryte += wykryteSlo[i];
		}
	}

	// Aktualizujemy nasz "notatnik" gry
	wykryteSlo = noweWykryte;

	// 3. Jeœli gracz trafi³ literê
	if (trafiony) {
		// Odœwie¿amy Label na ekranie (dodajemy spacje)
		String^ tekstDoWyswietlenia = "";
		for (int i = 0; i < wykryteSlo->Length; i++) {
			tekstDoWyswietlenia += wykryteSlo[i] + " ";
		}
		...
		// Sprawdzamy czy w haœle s¹ jeszcze jakieœ podkreœlniki. Jak nie ma = WYGRANA!
		if (!wykryteSlo->Contains("_")) {
			MessageBox::Show("Sigma ! You won !", " END OF THE GAME");
			button1->Enabled = false; // Wy³¹czamy przycisk Guess
		}
	}
	// 4. Jeœli gracz spud³owa³
	else {
		liczbaBledow++; // Odejmujemy ¿ycie (zwiêkszamy b³¹d)

		// Zmieniamy obrazek i napis z ¿yciami, jeœli nie przekroczyliœmy limitu
		if (liczbaBledow <= 6) {
			pictureBox1->Image = imageList1->Images[liczbaBledow];
			label1->Text = "Lives remaining: " + (6 - liczbaBledow).ToString();
		}

		// Sprawdzamy czy to by³ ostatni b³¹d = PRZEGRANA
		if (liczbaBledow == 6) {
	...
	// Na koniec po ka¿dym klikniêciu czyœcimy pole tekstowe, ¿eby od razu pisaæ now¹ literkê
	textBox1->Text = "";
	textBox1->Focus();
}
};
